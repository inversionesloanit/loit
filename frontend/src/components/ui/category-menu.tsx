...
function SidebarMenuItem({
  ...
  const { name, children: items, icon, slug } = item;
  return (
    <>
      <li
      ...
      >
        ...
        >
          {icon && (
            <div className="inline-flex w-8 shrink-0 3xl:h-auto">
              <Image
                src={icon.url ?? '/assets/placeholder/category-small.svg'}
        ...
        {Array.isArray(items) ? (
          <div className="overflow-auto absolute top-0 z-10 invisible hidden w-full h-full border rounded-md opacity-0 md:block left-full bg-brand-light border-border-base">
