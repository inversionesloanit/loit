    ...
    >
      ...
      <div className="flex items-start justify-between w-full overflow-hidden">
        <div className="ltr:pl-3 rtl:pr-3 md:ltr:pl-4 md:rtl:pr-4">
          <p className="block leading-5 transition-all text-brand-dark text-13px sm:text-sm lg:text-15px hover:text-brand">
            {item?.title}
          </p>
        ...
        <div className="flex grid font-semibold text-sm md:text-base text-brand-dark leading-5 shrink-0 min-w-[65px] md:min-w-[80px] !justify-end">
          {totalPrice}
          <div
            className="p-3 border border-1 border-border-one border-gray-400 bg-gray-100 border-opacity-70 rounded-full ml-2 hover:bg-border-one hover:bg-opacity-10 transition-colors duration-200 ease-in-out cursor-pointer"
            onClick={() => clearItemFromCart(item.id)}
          >
            <DeleteIcon />
          </div>
