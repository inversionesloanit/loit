...
const DownloadApps: React.FC<Props> = ({ className = 'pt-1.5 md:pt-0' }) => {
  const { appButtons, title, description } = data;
    ...
    return (
      <>
        {width > 1023 && (
          <div
            className={cn(
              'overflow-hidden bg-left-top bg-contain bg-no-repeat !bg-[url("/assets/images/app-download.webp")]',
              className
            )}
          >
